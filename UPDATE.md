...
- **章节标题弹窗**：进入剧情时左上角弹出卡图+章节名，1:1 复刻 enza 效果（含背景星光特效、扫光动画）。
- 播放器核心时序已精确对齐 enza（文字速度 10 cps、AUTO 等待 800ms）。
- 右下角控制面板默认收起，减少视觉干扰。
- 内置 Debug 面板，方便开发调试（按 ` 键呼出/隐藏）。

### 章节标题弹窗

进入剧情时，左上角会弹出如 enza 原版一样的章节标题卡片，显示卡图头像和章节名称。弹窗会自动滑入、停留约 2.5 秒后淡出消失。
...
如果你使用本项目播放并录制剧情内容，用于剧情汉化、视频投稿或相关展示，请在简介中标明使用了本项目而不是原游戏录制。
...
- **Chapter title popup**: card icon + chapter name overlay with sparkle background and sweep effects, 1:1 reverse-engineered from enza.
- Engine timing precisely aligned with enza (text speed 10 cps, AUTO wait 800ms).
- UI control panel defaults to collapsed state.
- Built-in **Debug panel** for development (toggle with `` ` `` key).

### Chapter Title Popup

When entering a scenario, a chapter title card (matching enza's original design) pops up in the top-left corner showing the card icon and chapter name. The popup slides in, holds for ~2.5s, then fades out.
...
`catIcon` maps to the event category icon from enza's `eventCategoryName`:
...
> Screenshot slots: `popup_idol.png`, `popup_produce.png`

### Debug Panel

Press `` ` `` (backtick) to toggle the debug overlay (top-right corner). It shows:
...
> Screenshot slot: `debug_panel.png`
